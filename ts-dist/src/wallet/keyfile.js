...
class KeyFile {
    ...
    async decrypt(password) {
        ...
        if (typeof window === "undefined" || window === null) {
            ...
            // For browser
            const argon2 = (await Promise.resolve().then(() => __importStar(require("argon2-browser"))));
        ...
        let entropy = await aesCipher.decrypt(Buffer.from(encrypted.substr(0, encrypted.length - 32), 'hex'), Buffer.from(aesNonce, 'hex'), Buffer.from(encrypted.substr(encrypted.length - 32, 32), 'hex'));
