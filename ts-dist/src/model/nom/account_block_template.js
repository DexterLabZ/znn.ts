...
class AccountBlockTemplate {
    constructor(version = 1, chainIdentifier = global_1.netId, blockType, hash = hash_1.emptyHash, previousHash = hash_1.emptyHash, height = 0, momentumAcknowledged = hash_height_1.emptyHashHeight, address = address_1.emptyAddress, toAddress = address_1.emptyAddress, amount = ethers_1.ethers.BigNumber.from("0"), tokenStandard = token_standard_1.TokenStandard.parse(token_standard_1.emptyTokenStandard), fromBlockHash = hash_1.emptyHash, data = Buffer.from([]), fusedPlasma = 0, difficulty = 0, nonce = "", publicKey = Buffer.from([]), signature = Buffer.from([])) {
    ...
    static fromJson(json) {
        return new AccountBlockTemplate(json["version"], json["chainIdentifier"], json["blockType"], hash_1.Hash.parse(json["hash"]), hash_1.Hash.parse(json["previousHash"]), json["height"], hash_height_1.HashHeight.fromJson(json["momentumAcknowledged"]), address_1.Address.parse(json["address"]), address_1.Address.parse(json["toAddress"]), ethers_1.ethers.BigNumber.from(json["amount"].toString()), token_standard_1.TokenStandard.parse(json["tokenStandard"]), hash_1.Hash.parse(json["fromBlockHash"]), json["data"] == null
            ? Buffer.from([])
            : json["data"] == ""
                ? Buffer.from([])
                : Buffer.from(json["data"], "base64"), json["fusedPlasma"], json["difficulty"], json["nonce"], (json["publicKey"] != null ? Buffer.from(json["publicKey"]) : Buffer.from([])), (json["signature"] != null ? Buffer.from(json["signature"]) : Buffer.from([])));
    }
    toJson() {
        return {
    ...
    static callContract(address, tokenStandard, amount, data) {
        const block = new AccountBlockTemplate(undefined, undefined, BlockTypeEnum.userSend, undefined, undefined, undefined, undefined, undefined, address, amount, tokenStandard, undefined, data, undefined, undefined, undefined, undefined);
        return block;
    }
    toString() {
