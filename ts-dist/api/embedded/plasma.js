...
class PlasmaApi {
    ...
    getPlasmaByQsr(qsrAmount) {
        return ethers_1.ethers.BigNumber.from(qsrAmount.toString()).mul(2100);
    }
    ...
    async fuse(beneficiary, amount) {
        // ToDo: Add response validation
        return account_block_template_1.AccountBlockTemplate.callContract(address_1.plasmaAddress, token_standard_1.qsrZts, amount, definitions_1.Definitions.plasma.encodeFunction("Fuse", [beneficiary.toString()]));
    }
    async cancel(id) {
        // ToDo: Add response validation
        return account_block_template_1.AccountBlockTemplate.callContract(address_1.plasmaAddress, token_standard_1.qsrZts, ethers_1.ethers.BigNumber.from("0"), definitions_1.Definitions.plasma.encodeFunction("CancelFuse", [id.getBytes()]));
    }
}
