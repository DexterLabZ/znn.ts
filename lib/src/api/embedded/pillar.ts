...
export class PillarApi{
  ...
  // RPC
  // 
  async getQsrRegistrationCost(): Promise<number>{
    // ToDo: Add response validation
  ...
  async getPillarsHistoryByEpoch(epoch: number, pageIndex: number = 0, pageSize: number = rpcMaxPageSize): Promise<PillarEpochHistoryList>{
  ...
  register(
    ...
    giveDelegateRewardPercentage: number = 100):AccountBlockTemplate{
