...
export class StakeApi{
  ...
  async getFrontierRewardByPage(address: Address, pageIndex = 0, pageSize = rpcMaxPageSize): Promise<RewardHistoryList> {
  ...
  // Contract methods
  stake(durationInSec: number, amount: number): AccountBlockTemplate{
    // ToDo: Add response validation
